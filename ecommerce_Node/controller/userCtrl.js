    ...
    let update = userCart.products.map((item) => {
      return {
        updateOne: {
          ...
          update : { $inc: { quantitiy : -item.count,sold: +item.count}},
        },
      };
    });
...
const getOrders= asyncHandler(async (req,res)=> {
  ...
  try{
   const userorders =await Order.findOne({ orderby : _id}).populate(
    "products.product").populate("orderby")
    .exec();
   res.json(userorders)
...
module.exports = {
  ...
  , handleRefreshToken, generateRefreshToken, logout, updatePassword, forgotPasswordToken, resetPassword, loginAdmin, getWishlist, saveAddress, userCart,
  getUserCart,emptyCart,applyCoupon,createOrder,getOrders,updateOrderStatus,getAllOrders
};
